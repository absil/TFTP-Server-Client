...
public class TftpServer {
	...
	private DatagramSocket serverSocket; 
	 ...
	 public void startServer()
	    {
		try {
	     serverSocket = new DatagramSocket(serverPort, serverAddress);			//creates a datagram socket for the server
		 System.out.println("TftpServer on port " + serverSocket.getLocalPort());	//prints the port that the server is looking on
	
		   while(true) {
			byte[] buf = new byte[1472];			
			DatagramPacket p = new DatagramPacket(buf, 1472);
			System.out.println("waiting for connection ....");
			serverSocket.receive(p);						//waits to receive a datagram packet
			TftpServerWorker worker = new TftpServerWorker(p);			//creates a new tftpServerWorkerThread which executes the rest of the server steps
			worker.start();
		    }
		}
		catch(Exception e) {
